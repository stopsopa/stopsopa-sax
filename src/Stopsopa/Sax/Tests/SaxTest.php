...
eos;
    ...
    public function testComplex() {
        ...
        $check = <<<eos
["raz", 45, ["dwa"], {"trzy":"cztery"}, null, true, false]
eos;

        $check = json_decode($check, true);

        fwrite(STDOUT, $this->_var_dump('json_decode1'));
        fwrite(STDOUT, $this->_var_dump($check));

        $check = <<<eos
["raz", 45, ["dwa"], {"tr\"zy":"cztery"}, null, true, false]
eos;

        $check = json_decode($check, true);

        fwrite(STDOUT, $this->_var_dump('json_decode2'));
        fwrite(STDOUT, $this->_var_dump($check));
